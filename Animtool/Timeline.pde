...
class Timeline {
  ...
  Timeline(int targetHeight) {
    ...
    handle[1] = new Handle();
    handle[1].time = duration;
    handle[1].value = minValue + (maxValue-minValue)/2f;
  ...
  private void initLayers() {
    duration = handle[handle.length-1].time;
  ...
  private void updateForeground() {
    ...
    for (int i=0; i < handle.length-1; i++) {
      foreground.line(timeToLocalX(handle[i].time), valueToLocalY(handle[i].value), timeToLocalX(handle[i+1].time), valueToLocalY(handle[i+1].value));
    }
  ...
  //TODO
  //this will recreate the PGraphics objects, to deal with new width
  ...
  float getValueAtTime(float time) {
    if (time >= duration) return handle[handle.length-1].value;
    if (time <= 0) return handle[0].value;
    ...
    for (int i= handle.length-2; i >= 0; i--) {
      if (time > handle[i].time) {
        ...
        float interpTimePercent = (time - handle[i].time) / (handle[i+1].time - handle[i].time);
